...
%   objective intelligibility measure described in [1], where x and y
%   denote the clean and processed speech, respectively, with samplerate
%   fs_signal in Hz. The output d is expected to have a monotonic relation
%   with the subjective speech-intelligibility, where a higher d denotes
%   better intelligible speech. See [1] for more details.
%
%   References:
...
for i = 1:size(x_hat, 2)
    X(:, i)	= sqrt(H*abs(x_hat(:, i)).^2);      % apply 1/3 octave bands as described in Eq.(1)
...
for m = N:size(X, 2)
    ...
    aY_seg 	= Y_seg.*repmat(alpha, [1 N]);                               	% obtain \alpha*Y_j(n) from Eq.(2)
    for j = 1:J
      	Y_prime             = min(aY_seg(j, :), X_seg(j, :)+X_seg(j, :)*c); % apply clipping from Eq.(3)
        d_interm(j, m-N+1)	= corr(X_seg(j, :).', Y_prime(:));           	% obtain correlation coeffecient from Eq.(4)
    end
end
        
d = mean(d_interm(:));                                                      % combine all intermediate intelligibility measures as in Eq.(4)
