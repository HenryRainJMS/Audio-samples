function [W,H,errs]=nmf_spp(V,r,varargin)
[m,n]=size(V);
% process arguments
...
load('data/Ts.mat');
alpha=2;
theta=(Ts).^-1; %super parameter
for t=1:niter
    %update W
    W = W0 .* ( ((R.^(beta-2) .* V)*H'+(alpha-1)./W0) ./ max(R.^(beta-1)*H'+theta, myeps) );
...
%     end
    ...
    R = W*H;
    % update H
    H = H .* ( (W'*(R.^(beta-2) .* V)) ./ max(W'*R.^(beta-1)+lambda*p*H.^(p-1), myeps) );
